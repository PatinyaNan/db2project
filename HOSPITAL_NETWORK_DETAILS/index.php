    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $('#add_button').click(function(){
                ...
                $('.modal-title').text("Add Hospital Details");
                $('#action').val("Add");
                $('#operation').val("Add");
            });
            ...
            window.handleUpdate = function(id){
                $.ajax({
                    ...
                    success: function(resp){
                        $('#ID_HOSPITAL_NETWORK_DETAILS').val(resp.ID_HOSPITAL_NETWORK_DETAILS);
                        $('#ID_HOSPITAL_NETWORK').val(resp.ID_HOSPITAL_NETWORK);
                        $('#ID_INSURANCE').val(resp.ID_INSURANCE);
                        $('.modal-title').text("Edit Hospital Network Details");
                        $('#product_id').val(id);
                        $('#action').val("Edit");
                        $('#operation').val("Edit");
            ...
            $.get('fetch.php', (data) => {
                ...
                const html = fetchData.rows.map((a, b) => {
                    return '<tr id="item-'+a.ID_HOSPITAL_NETWORK_DETAILS+'">\n\
                        <td>'+String(a.ID_HOSPITAL_NETWORK_DETAILS)+'</td>\n\
                        <td>'+a.ID_HOSPITAL_NETWORK+'</td>\n\
                        <td>'+a.ID_INSURANCE+'</td>\n\
                        <td>\n\
                            <button type="button" onclick="window.handleUpdate(\''+a.ID_HOSPITAL_NETWORK_DETAILS+'\')" class="btn btn-warning btn-xs update" id="product-edit-'+a.ID_HOSPITAL_NETWORK_DETAILS+'" data-row-id="'+a.ID_HOSPITAL_NETWORK_DETAILS+'"><span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;Edit</button>\n\
                        </td>\n\
                        <td>\n\
                            <button type="button" onclick="window.handleDelete(\''+a.ID_HOSPITAL_NETWORK_DETAILS+'\')" class="btn btn-danger btn-xs delete" id="product-delete-'+a.ID_HOSPITAL_NETWORK_DETAILS+'" data-row-id="'+a.ID_HOSPITAL_NETWORK_DETAILS+'"><span class="fa fa-trash-o"></span>&nbsp;&nbsp;Delete</button>\n\
                        </td>\n\
                    </tr>';
         ...
         $(document).on('submit', '#product_form', function(event){
              ...
              var ID_HOSPITAL_NETWORK_DETAILS = $('#ID_HOSPITAL_NETWORK_DETAILS').val();
              var ID_HOSPITAL_NETWORK = $('#ID_HOSPITAL_NETWORK').val();
              var ID_INSURANCE = $('#ID_INSURANCE').val();
              var form_data = $(this).serialize();
                  if(ID_HOSPITAL_NETWORK_DETAILS != '' && ID_HOSPITAL_NETWORK != '' && ID_INSURANCE != '')
                  {
                       $.ajax({
                        url:"insert.php",
                        method:"POST",
    ...
    <body>
        <div class="container box">
            <div class="pull" align="right">
                <h1 align="center"><b>ระบบการจัดการรายละเอียดโรงพยาบาลในเครือ</b></h1>
            </div>
            <div class="container" align="right">
                ...
                <div class="table-responsive">
                 <table id="myTable"  class="table table-striped table-bordered" cellspacing="0" width="100%">
                  <thead>
                       <tr class="bg-primary">
                            <td>ID_HOSPITAL_NETWORK_DETAILS</td>
                            <td>ID_HOSPITAL_NETWORK</td>
                            <td>ID_INSURANCE</td>
          ...
          <form method="post" id="product_form">
               <div class="modal-content">
                    <div class="container-fluid">
                        ...
                        <h4 class="modal-title">Add Hospital Network Details</h4>
                    </div>
                    <div class="modal-body">
                        <label>ID Hospital Network Details</label>
                            <input type="text" name="ID_HOSPITAL_NETWORK_DETAILS" id="ID_HOSPITAL_NETWORK_DETAILS" class="form-control" value='<?php echo $id_hos_details;?>' disabled/><br />
                        <label>Select Hospital Network</label>
                              <select name="ID_HOSPITAL_NETWORK" id="ID_HOSPITAL_NETWORK" class="form-control">
                                   <option value="">Hospital Network</option>
                                   <?php echo $output1; ?>
                              </select> <br />
