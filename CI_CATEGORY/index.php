    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $('#add_button').click(function(){
                ...
                $('.modal-title').text("Add Ci Category");
                $('#action').val("Add");
                $('#operation').val("Add");
            });
            ...
            window.handleUpdate = function(id){
                $.ajax({
                    ...
                    success: function(resp){
                        ...
                        $('#CI_NAME').val(resp.CI_NAME);
                        $('.modal-title').text("Edit Ci Category");
                        $('#product_id').val(id);
                        $('#action').val("Edit");
                        $('#operation').val("Edit");
            ...
            $.get('fetch.php', (data) => {
                ...
                const html = fetchData.rows.map((a, b) => {
                    return '<tr id="item-'+a.ID_CATEGORY_CI+'">\n\
                        <td>'+String(a.ID_CATEGORY_CI)+'</td>\n\
                        <td>'+a.CI_NAME+'</td>\n\
                        <td>\n\
                            <center><button type="button" onclick="window.handleUpdate(\''+a.ID_CATEGORY_CI+'\')" class="btn btn-warning btn-xs update" id="product-edit-'+a.ID_CATEGORY_CI+'" data-row-id="'+a.ID_CATEGORY_CI+'"><span class="fa fa-pencil-square-o"></span>&nbsp;&nbsp;Edit</button>\n\
                            <button type="button" onclick="window.handleDelete(\''+a.ID_CATEGORY_CI+'\')" class="btn btn-danger btn-xs delete" id="product-delete-'+a.ID_CATEGORY_CI+'" data-row-id="'+a.ID_CATEGORY_CI+'"><span class="fa fa-trash-o"></span>&nbsp;&nbsp;Delete</button></center>\n\
                        </td>\n\
                    </tr>';
    ...
    <body>
        <div class="container box">
            <div class="pull" align="right">
                <h1 align="center"><b>ระบบการจัดการประเภท Ci</b></h1>
            </div>
          ...
          <form method="post" id="product_form">
               <div class="modal-content">
                    <div class="container-fluid">
                        ...
                        <h4 class="modal-title">Add Ci Category</h4>
                    </div>
                    <div class="modal-body">
                        <label>ID Ci Category</label>
                            <input type="text" name="ID_CATEGORY_CI" id="ID_CATEGORY_CI" class="form-control" value='<?php echo $id_ci_category;?>' disabled/><br />
                        <label>Enter Ci Name</label>
                            <input type="text" name="CI_NAME" id="CI_NAME" class="form-control" /><br />
                    </div>
